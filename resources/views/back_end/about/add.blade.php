...
@section('content')
    @php
        ...
        $typeInput = ['text' , 'textarea' , 'text' , 'text' , 'text' , 'textarea'];
    @endphp
    <div class="card mb-4 mt-2">
        ...
        <div class="card-body">
            <form action="{{ route('about.store') }}" method="post" class="mb-3" enctype="multipart/form-data">
                @csrf
                @foreach($nameTitle as $key=>$name)
                    ...
                    @if($typeInput[$key] === 'textarea')
                        <textarea name="{{ $nameInput[$key] }}" id="editor"
                                  class="ckeditors_ledat">{{ old($nameInput[$key]) }}</textarea>
                        @error($nameInput[$key])
                ...
                <div class="col-5 mt-4 me-0 pl-0">
                    <input type="submit" value="Thêm" class="btn btn-danger me-0"/>
                </div>
            </form>
        </div>
    </div>
